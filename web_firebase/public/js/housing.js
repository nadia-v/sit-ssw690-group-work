...
.get().then(snapshot => {
    ...
    storageRef.getDownloadURL().then(function(url) {  
        ...
        housing_posts.innerHTML += '<div class="col-lg-8 mb-4">';
        housing_posts.innerHTML += '<div class="card shadow mb-4">';
        housing_posts.innerHTML += '<div class="card-header py-3">';
        housing_posts.innerHTML += '<h3 class="m-0 font-weight-bold text-primary"<b>' + doc.data().adTitle + '</b></h3>';
        housing_posts.innerHTML += '</div>';
        housing_posts.innerHTML += '<div class="card-body">';
        housing_posts.innerHTML += '<div class="text-center">';
        housing_posts.innerHTML += '<img class="img-fluid px-3 px-sm-4 mt-3 mb-4" style="width: 25rem"; id="housing-post-image" src=' + url + 'alt="">';
        housing_posts.innerHTML += '<p><b>Category: </b>'+ doc.data().adCategory +'</p>'; 
        housing_posts.innerHTML += '<p><b>Date: </b>' + myDate +'</p>';   
        housing_posts.innerHTML += '<p><b>Description: </b>' + doc.data().adDescription +'</p>';   
        housing_posts.innerHTML += '<p><b>Rent Price: </b>' + doc.data().adRent +'</p>';   
        housing_posts.innerHTML += '<p><b>Status: </b>' + doc.data().adStatus +'</p></div>';
        housing_posts.innerHTML += '<ul id="posts"></ul>';
        housing_posts.innerHTML += '<div class="card-footer py-3">';  
        housing_posts.innerHTML += '<a target="_blank" rel="nofollow" href="mailto:' + doc.data().emailUser + '"class="btn btn-primary my-2">Email Me</a></div></div></div></div>';
